...
object MaterializingStreams extends App {
  ...
  // source.runWith(sink) <=>  source.toMat(sink)(Keep.right).run()
  val sumFuture = source.runWith(sink)
  val anotherSumFuture = source.toMat(sink)(Keep.right).run()

  sumFuture.onComplete {
    case Success(value) => println(s"[sumFuture] The sum of all elements is :$value")
    case Failure(ex) => println(s"[sumFuture] The sum of the elements could not be computed: $ex")
  }

  anotherSumFuture.onComplete {
    case Success(value) => println(s"[anotherSumFuture] The sum of all elements is :$value")
    case Failure(ex) =>
      println(s"[anotherSumFuture] The sum of the elements could not be computed: $ex")
  }

  // BY DEFAULT - WHEN YOU ARE USING VIA & TO TO COMPOSE A RUNNABLE GRAPH,
  // THE MATERIALIZED VALUE IS THE ONE OF THE LEFT-MOST COMPONENT
  ...
  // viaMat/toMat (the Mat suffix) let you specify materialize combiner
  ...
  val graph2: RunnableGraph[NotUsed] =
    simpleSource.viaMat(simpleFlow)(Keep.right).toMat(simpleSink)(Keep.left)
  ...
  // both ways
  val materializedValue: (NotUsed, Future[Done]) = Flow[Int].map(x => 2 * x).runWith(simpleSource, simpleSink)
